...
const Page = () => {
    ...
    const handleContinue = () => {
        const newPrcnt = (100 / data.length) * (currentIndex + 1);
            ...
            <div className={`footer mt-56 ${isCorrect === true ? 'bg-green-200' : isCorrect === false ? 'bg-red-300' : ''}`}>
                ...
                {isCorrect === false && (
                    <div className='mt-10 flex text-xl'>
                        {`Option ${String.fromCharCode(65 + data[currentIndex]?.correct_answer_indices[0])}. ${data[currentIndex]?.answers[data[currentIndex]?.correct_answer_indices[0]]}`}
                    </div>
                )}
